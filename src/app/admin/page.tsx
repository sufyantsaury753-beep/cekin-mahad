...
function AdminDashboardContent() {
  ...
  // Move & Evict Mahasantri Modal State
  ...
  // Handle Keluarkan Mahasantri dari Kamar (Bebaskan ranjang agar santri bisa pilih ulang)
  const handleEvictMahasantri = (nimNisn: string, nama: string, nomorKamar: string, bedNumber: number) => {
    if (
      confirm(
        `⚠️ KONFIRMASI PENGELUARAN DARI KAMAR:\n\nApakah Anda yakin ingin mengeluarkan ${nama} (${nimNisn}) dari Kamar ${nomorKamar} (Bed ${bedNumber})?\n\n• Ranjang ${nomorKamar} Bed ${bedNumber} akan langsung kosong kembali.\n• Santri bersangkutan dapat memilih kamar baru secara mandiri di formulir pendaftaran.`
      )
    ) {
      const res = MahadStore.evictMahasantriRoom(nimNisn);
      if (res.success) {
        setNotification(`Mahasantri ${nama} (${nimNisn}) berhasil dikeluarkan dari Kamar ${nomorKamar} Bed ${bedNumber}. Ranjang kini siap dipilih kembali.`);
        setShowMoveModal(false);
        setSelectedMhsForMove(null);
      } else {
        alert(res.error || 'Gagal mengeluarkan mahasantri.');
      }
    }
  };

  // Handle Pindah Kamar Langsung oleh Admin
  const handleDirectMove = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedMhsForMove || !moveTargetKamarId) {
      alert('Silakan pilih kamar tujuan terlebih dahulu.');
      return;
    }

    const targetRoom = rooms.find((r) => r.id === moveTargetKamarId);
    if (!targetRoom) {
      alert('Kamar tujuan tidak ditemukan.');
      return;
    }

    const res = MahadStore.moveMahasantriRoom(selectedMhsForMove.nimNisn, moveTargetKamarId, moveTargetBed);
    if (res.success) {
      setNotification(
        `Mahasantri ${selectedMhsForMove.nama} berhasil dipindahkan ke ${targetRoom.gedung} Kamar ${targetRoom.nomor} (Bed ${moveTargetBed})!`
      );
      setShowMoveModal(false);
      setSelectedMhsForMove(null);
    } else {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                        ...
                        <td className="py-3.5 px-4 text-center">
                          <div className="flex items-center justify-center gap-1.5 flex-wrap">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                setSelectedMhsForMove(mhs);
                                const availableRooms = rooms.filter(
                                  (r) => !r.isLocked && (r.gender === mhs.jenisKelamin || !r.gender)
                                );
                                if (availableRooms.length > 0) {
                                  setMoveTargetKamarId(availableRooms[0].id);
                                  const emptyBed = availableRooms[0].beds.find((b) => !b.isOccupied);
                                  setMoveTargetBed(emptyBed ? emptyBed.bedNumber : 1);
                                }
                                setShowMoveModal(true);
                              }}
                              className="px-2.5 py-1.5 bg-amber-50 text-amber-800 hover:bg-amber-100 border border-amber-200 rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all"
                              title="Pindahkan ke kamar lain atau keluarkan santri dari kamar"
                            >
                              <ArrowRightLeft className="w-3 h-3" />
                              <span>Pindah / Keluarkan</span>
                            </button>
      ...
      {/* ========================================================= */}
      {/* MODAL 5: PINDAH / KELUARKAN MAHASANTRI DARI KAMAR          */}
      {/* ========================================================= */}
      {showMoveModal && selectedMhsForMove && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl border border-slate-200 space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-base text-slate-800">Pindah / Keluarkan Mahasantri</h3>
                  <p className="text-xs text-slate-500">Relokasi kamar atau bebaskan ranjang santri.</p>
                </div>
              </div>
              <button
                onClick={() => {
            ...
            {/* Info Mahasantri Saat Ini */}
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-2">
              <div className="flex justify-between items-start">
                <div>
                  <div className="font-bold text-slate-900 text-sm">{selectedMhsForMove.nama}</div>
                  <div className="font-mono text-emerald-800 text-xs">{selectedMhsForMove.nimNisn}</div>
                  <div className="text-[11px] text-slate-500">{selectedMhsForMove.fakultas} &bull; {selectedMhsForMove.jurusan}</div>
                </div>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                  selectedMhsForMove.jenisKelamin === 'L' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800'
                }`}>
                  {selectedMhsForMove.jenisKelamin === 'L' ? '👦 Putra' : '🧕 Putri'}
                </span>
              </div>
              
              <div className="pt-2 border-t border-slate-200 flex items-center justify-between text-xs">
                <span className="text-slate-500 font-medium">Posisi Saat Ini:</span>
                <span className="font-bold text-uin-primary bg-uin-primary/10 px-2.5 py-1 rounded-lg">
                  {selectedMhsForMove.gedung} &bull; Kamar {selectedMhsForMove.nomorKamar} (Bed {selectedMhsForMove.bedNumber})
                </span>
              </div>
            </div>

            {/* OPSI 1: PINDAH KE KAMAR LAIN OLEH ADMIN */}
            <form onSubmit={handleDirectMove} className="space-y-3 bg-white p-4 rounded-2xl border border-emerald-200 bg-emerald-50/20">
              <div className="font-bold text-xs text-emerald-950 flex items-center gap-1.5">
                <BedDouble className="w-4 h-4 text-emerald-700" />
                <span>Opsi 1: Pindahkan Langsung ke Kamar Baru</span>
              </div>
              <p className="text-[11px] text-slate-500">
                Pilih kamar dan ranjang tujuan yang masih tersedia untuk santri ini.
              </p>

              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 mb-1">Kamar Tujuan:</label>
                  <select
                    value={moveTargetKamarId}
                    onChange={(e) => {
                      setMoveTargetKamarId(e.target.value);
                      const target = rooms.find((r) => r.id === e.target.value);
                      if (target) {
                        const emptyBed = target.beds.find((b) => !b.isOccupied);
                        setMoveTargetBed(emptyBed ? emptyBed.bedNumber : 1);
                      }
                    }}
                  ...
                  >
                    {rooms
                      .filter((r) => !r.isLocked && (r.gender === selectedMhsForMove.jenisKelamin || !r.gender))
                      .map((r) => {
                        const emptyBedsCount = r.beds.filter((b) => !b.isOccupied).length;
                        return (
                          <option key={r.id} value={r.id}>
                            {r.gedung.replace("Ma'had ", "")} - Kamar {r.nomor} ({emptyBedsCount} kosong)
                          </option>
                        );
                      })}
                  </select>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-700 mb-1">Nomor Ranjang:</label>
                  <select
                    value={moveTargetBed}
                    onChange={(e) => setMoveTargetBed(Number(e.target.value))}
                    className="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs outline-none font-semibold"
                  >
                    {(() => {
                      const curRoom = rooms.find((r) => r.id === moveTargetKamarId);
                      if (!curRoom) return <option value={1}>Bed 1</option>;
                      return curRoom.beds.map((b) => (
                        <option key={b.bedNumber} value={b.bedNumber} disabled={b.isOccupied && b.mahasantriNimNisn !== selectedMhsForMove.nimNisn}>
                          Bed {b.bedNumber} {b.isOccupied ? (b.mahasantriNimNisn === selectedMhsForMove.nimNisn ? '(Ranjang Ini)' : '(Penuh)') : '(Tersedia)'}
                        </option>
                      ));
                    })()}
                  </select>
              ...
              <button
                type="submit"
                className="w-full py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5"
              >
                <ArrowRightLeft className="w-3.5 h-3.5" />
                <span>Simpan Pemindahan Kamar</span>
              </button>
            </form>

            {/* OPSI 2: KELUARKAN SANTRI / BEBASKAN RANJANG */}
            <div className="space-y-2.5 bg-rose-50/50 p-4 rounded-2xl border border-rose-200">
              <div className="font-bold text-xs text-rose-950 flex items-center gap-1.5">
                <UserMinus className="w-4 h-4 text-rose-600" />
                <span>Opsi 2: Keluarkan Santri dari Kamar Ini</span>
              </div>
              <p className="text-[11px] text-slate-500 leading-relaxed">
                Ranjang <strong>Kamar {selectedMhsForMove.nomorKamar} (Bed {selectedMhsForMove.bedNumber})</strong> akan langsung dikosongkan. Santri tetap terdaftar di SK dan dapat memilih kamar baru secara mandiri di formulir santri.
              </p>

              <button
                type="button"
                onClick={() =>
                  handleEvictMahasantri(
                    selectedMhsForMove.nimNisn,
                    selectedMhsForMove.nama,
                    selectedMhsForMove.nomorKamar,
                    selectedMhsForMove.bedNumber
                  )
                }
              ...
              >
                ...
                <span>Keluarkan &amp; Bebaskan Ranjang Sekarang</span>
              </button>
            </div>
